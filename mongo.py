...
@app.route('/users/update/<id>',methods=['POST'])  #for update the user (username, password )
def update(id):
    if request.method == 'POST':
        ...
        resp = jsonify('you have updated the user!')
        return resp
...
def post():
    if request.method == 'POST':
        ...
        post = posts.insert({"title":title,"content":content,"category":category,'file':file.filename,'user_id':the_user['_id'],'username':the_user['username']})
...
@app.route('/file')  #to download the file given a filename
def file():
